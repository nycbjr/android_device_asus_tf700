...
def main():
    try:
        # parse arguments
        usage = "Usage: %prog <options>\nThe script captures image with AE, AWB and AF set to Auto by default."
        ...
        parser.add_option('-p', '--preview', dest='preview', default=(0, 0), type="int", metavar="PREVIEW_RES",
        ...
        parser.add_option('-s', '--still', dest='still', default=(0, 0), type="int", metavar="STILL_RES",
                    nargs=2, help = 'set still resolution (--still=<width> <height>)\n\
                                     , default is maximum supported still resolution')
        ...
        parser.add_option('-n', '--name', dest='fname', default="test", type="str", metavar="FILE_NAME",
        ...
        # capture an image
        oCamera.still(options.output_dir + "/" + options.fname + ".jpg")
