...
class TestFactory:
    ...
    def getTestObject(self, testName, options):
        if (testName == "jpeg_capture"):
            return nvcstest.NvCSJPEGCapTest()
        elif (testName == "raw_capture"):
            return nvcstest.NvCSRAWCapTest()
        elif (testName == "concurrent_raw"):
            return nvcstest.NvCSConcurrentRawCapTest()
        elif (testName == "exposuretime"):
            return nvcstest.NvCSExposureTimeTest()
        elif (testName == "gain"):
            return nvcstest.NvCSGainTest()
        elif (testName == "focuspos"):
            return nvcstest.NvCSFocusPosTest()
        elif (testName == "multiple_raw"):
            return nvcstest.NvCSMultipleRawTest()
        else:
            return None
...
def main():
    ...
    logger = nvcstest.Logger()

    # parse arguments
    usage = "Usage: %prog <options>"
    parser = OptionParser(usage)

    parser.add_option('-s', dest='sanity', default=False, action="store_true",
            help = 'Run Sanity tests')

    parser.add_option('-t', dest='test_names', default=None, type="string", action = "append",
            help = 'Test name to execute', metavar="TEST_NAME")

    parser.add_option('-d', dest='disabled_test_names', default=None, type="string", action = "append",
            help = 'Disable the test', metavar="TEST_NAME")

    parser.add_option('-i', dest='imager_id', default=0, type="int",
    ...
    # set the imager id
    nvcstest.gImagerId = options.imager_id
    ...
    elif (options.test_names != None):
        testNamesList = options.test_names

    __validateTestNames(testNamesList)

    # create a dictionary for disabled test names
    if (options.disabled_test_names != None):
        for testName in options.disabled_test_names:
            # get the test object
            if (TestFactory.hasTest(testName)):
                TestFactory.disableTest(testName)

    logger.info("Imager ID: %d" % options.imager_id)
    for testName in testNamesList:
        ...
        try:
            ...
            if (TestFactory.isEnabled(testName)):
                ...
                testOb = TestFactory.getTestObject(testName, options)
                retVal = testOb.run()
